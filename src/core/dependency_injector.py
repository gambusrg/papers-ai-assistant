...
from src.adapters.output.sqlite import SQLiteConversationRepository
...
llm = GroqLLM(
    client=Groq(
        api_key=os.getenv("GROQ_API_KEY"), http_client=httpx.Client(verify=False)
    )
)

conversation_repo = SQLiteConversationRepository(db_path="papers-ai.db")
